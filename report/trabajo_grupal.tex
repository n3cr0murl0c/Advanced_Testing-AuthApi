...
\textit{Corrección aplicada:} el check de blacklist fue removido de \texttt{OnTokenValidated}
y reubicado en un middleware inline entre \texttt{UseAuthentication()} y
\texttt{UseAuthorization()}. Este middleware inspecciona \texttt{HttpContext.User} —
ya construido por el handler JWT — y retorna HTTP~401 con
\texttt{ErrorResponse("TOKEN\_REVOKED",~...)} si el \texttt{jti} está revocado,
cortando el pipeline antes de que \texttt{UseAuthorization()} evalúe el \texttt{[Authorize]}.
...
\begin{enumerate}[itemsep=4pt]
    ...
    \item \textbf{Corrección aplicada para DEF-02:} el check de blacklist fue removido de \texttt{OnTokenValidated} y reubicado en un middleware inline entre \texttt{UseAuthentication()} y \texttt{UseAuthorization()} en \texttt{Program.cs}. El middleware inspecciona \texttt{HttpContext.User} — ya construido por el handler JWT — y retorna HTTP~401 con \texttt{ErrorResponse("TOKEN\_REVOKED",~...)} si el \texttt{jti} está en la blacklist. BN-16 ahora retorna HTTP~401 correctamente.
\end{enumerate}
