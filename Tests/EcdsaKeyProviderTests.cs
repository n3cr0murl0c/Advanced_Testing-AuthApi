...
public sealed class EcdsaKeyProviderTests : IDisposable
{
    ...
    private IConfiguration BuildConfig(string keyPath) =>
        new ConfigurationBuilder()
            .AddInMemoryCollection(
                new Dictionary<string, string?> { ["Jwt:EcdsaKeyPath"] = keyPath }
            )
            .Build();
    ...
    public void Constructor_WhenKeyFileExists_LoadsSameKey()
    {
        ...
        // First boot: generate
        using var p1 = new EcdsaKeyProvider(
            BuildConfig(keyPath),
            NullLogger<EcdsaKeyProvider>.Instance
        );
        ...
        // Second boot: reload
        using var p2 = new EcdsaKeyProvider(
            BuildConfig(keyPath),
            NullLogger<EcdsaKeyProvider>.Instance
        );
    ...
    public void SigningCredentials_AlgorithmIsES256()
    {
        ...
        provider
            .SigningCredentials.Algorithm.Should()
            .Be(SecurityAlgorithms.EcdsaSha256, "ES256 is the only accepted algorithm");
    }
    ...
    public void GeneratedKey_CurveIsP256()
    {
        ...
        var @params = provider.PublicSecurityKey.ECDsa.ExportParameters(
    ...
    public void SignAndVerify_RoundTrip_Succeeds()
    {
        ...
        publicKey
            .VerifyData(payload, signature, HashAlgorithmName.SHA256)
            .Should()
            .BeTrue("signature created with private key must verify with matching public key");
    }
